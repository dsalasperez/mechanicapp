...
class Database:
    ...
    def get_mechanic_name(self, mechanic_id):
        query = "SELECT name FROM mechanics WHERE mechanic_id = %s"
        values = (mechanic_id,)
        self.cursor.execute(query, values)
        result = self.cursor.fetchone()
        if result is not None:
...
class RegisterScreen(Screen):
    ...
    def register(self, instance):
        ...
        user_id = self.database.create_user(username, password, name)
...
class DataScreen(Screen):
    ...
    def submit_data(self, instance):
        ...
        date_replaced_str = self.date_replaced_input.text
        comments = self.comments_input.text

        # Validate date format
        try:
            date_replaced = date.fromisoformat(date_replaced_str)
        except ValueError:
            print('Invalid date format. Please use YYYY-MM-DD')
            return

        # Insert the data into the database, including the mechanic's ID and the current timestamp
        self.database.insert_data(part_number, price, description, vehicle, customer_name, mechanic_id)
    ...
    def get_mechanic_name(self):
        ...
        try:
            ...
            print("An error occurred when fetching mechanic name:", e)
            return None
